(function( w ){
    ...
    */
    ...
    function computeAspectRatio(_w, _h) {
      var bestFit = 100;
      // Just in case something is really fucked up, let's default to the middle image.
      var ratio = RATIOS[Math.round((RATIOS.length - 1) / 2)];  
      if (_w !== 0 && _h !== 0) {
        var aspectRatio = _w/_h;
        for (var i in RATIOS) {
          var fitDelta = Math.abs(aspectRatio - RATIOS[i][1]) / RATIOS[i][1];
          if (fitDelta < bestFit) {
            bestFit = fitDelta;
            ratio = RATIOS[i][0];
          }
